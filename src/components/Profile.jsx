...
function Profile() {
  ...
  return (
    <div className='w-[98%] max-h-[70%] rounded bg-gradient-to-br from-green-700 to-blue-500 relative flex justify-center items-center'>
      <div className='w-screen min-h-full flex justify-center items-center border-white border-2'>
          <div className=' flex w-96 h-96  flex-col items-center justify-center gap-2'>
            <div id="profile-pic" className='w-48 h-36 flex absolute top-10'>
              ...
              <div id='edit-profile-pic' className='w-6 h-6 relative mt-auto' onClick={handleFileinput} >
                  <svg width="48" height="48" viewBox="0 0 48 48" className='w-6 h-6 cursor-pointer hover:scale-110' fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            <div className='flex flex-col w-fit h-fit p-8 border-2 border-white border-double gap-8 text-slate-100 text-2xl rounded-3xl'>
            <h2>Your username: {username}</h2>
            <h2>Your email: {email}</h2>
            </div>
            <button onClick={popVisible} className='hover:scale-x-150 transition-all duration-1000 w-48 bg-slate-500 p-2 rounded-xl hover:bg-[#212121] hover:text-white active:bg-white active:text-black' >Edit Details</button>
          </div>
          <div className={`ml-4 overflow-auto h-96 w-96  border-2 resize border-black p-6 rounded-xl relative bg-white text-black
           ${visiblePop ? 'block' : 'hidden'}`}>
            <button onClick={popVisible} className='absolute top-0 right-0 bg-slate-500 p-2 rounded-xl text-red-700 active:border-2 active:text-white'>Close</button>
            <form onSubmit={editDetails} className='w-[90%] h-[80%] absolute top-10 flex flex-col items-center justify-around'>
              <label htmlFor="username">Username</label>
              <input type="text" id="username" value={newUsername} onChange={(e)=>setNewUsername(e.target.value)}
              className='border-b-2 border-slate-600 w-full active:border-b-2 active:border-black h-auto text-center text-2xl' />
              <label htmlFor="email">Email</label>
              <input type="email" id="email" value={newEmail} onChange={(e)=>setNewEmail(e.target.value)}
              className='border-b-2 border-slate-600 w-full active:border-b-2 active:border-black h-auto text-center text-2xl' />
              <button className=' w-48 bg-slate-500 p-2 rounded-xl hover:text-white  active:text-black hover:bg-green-700' type="submit">Edit details</button>
            </form>
           </div>
      </div>
    </div>
